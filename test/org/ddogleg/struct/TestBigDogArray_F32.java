 ...
 * @author Peter Abeles
 */
public class TestBigDogArray_F32 extends ChecksBigDogArray<float[]> {
	...
	private void checkResizeValue( BigDogGrowth growth ) {
		...
		var alg = new BigDogArray_F32(1, blockSize, growth);

		// give it some values that we know and this is within the first block
		alg.resize(blockSize - 4, (float)4);
		alg.resize(blockSize - 1, (float)3);
		...
		alg.resize(blockSize + 3, (float)3);
		...
		alg.fill(0, alg.size, (float)5);
		// Resize past the second block
		alg.resize(blockSize*2 + 1, (float)6);
	...
	private void checkAppendArray( int initialSize, BigDogGrowth growth, int arraySize ) {
		...
		var alg = new BigDogArray_F32(initialSize, 10, growth);
	...
	private void checkFill( BigDogGrowth growth ) {
		var alg = new BigDogArray_F32(1, 10, growth);
		alg.resize(4, (float)2);
		alg.resize(11, (float)10);
		alg.fill(0, 3, (float)1);
		alg.fill(3, 9, (float)2);
	...
	@Test void setArray() {
		var alg = new BigDogArray_F32(2, 10, BigDogGrowth.GROW);
		alg.resize(25);
	...
	@Test void forEach() {
		...
		DogArray_F32 results = new DogArray_F32();
	...
	@Test void forIdx() {
		...
		DogArray_F32 results = new DogArray_F32(12);
		results.resize(12);
	...
	@Test void applyIdx() {
		...
		alg.applyIdx(1, 12, ( idx, val ) -> (float)(idx*2));
		for (int i = 0; i < alg.size; i++) {
			if (i >= 1 && i < 12) {
	...
	@Test void processByBlock() {
		...
		alg.processByBlock(1, alg.size - 1, ( block, idx0, idx1, offset ) -> {
			for (int i = idx0; i < idx1; i++) {
				block[i] = (float)(i - idx0 + offset);
			}
		});
		for (int i = 0; i < alg.size; i++) {
			if (i >= 1 && i < alg.size - 1) {
				assertEquals((float)(i - 1), alg.get(i));
			} else {
				assertEquals(0, alg.get(i));
			}
		}
	...
	@Test void isEquivalent() {
		...
		var a = new BigDogArray_F32(10, 5, BigDogGrowth.GROW);
		var b = new BigDogArray_F32(10, 4, BigDogGrowth.GROW);

		// see if it handles an empty array correctly
		assertTrue(a.isEquivalent(b, 0.0f));
		...
		a.set(4, 4 + 0.1f);
		assertFalse(a.isEquivalent(b, 0.0f));
		assertFalse(b.isEquivalent(a, 0.0f));

		// increase tolerance
		assertTrue(a.isEquivalent(b, 0.1f));
		assertTrue(b.isEquivalent(a, 0.1f));

		// make the shapes not match
		b.append(4);
		assertFalse(a.isEquivalent(b, 0.1f));
	...
	@Override public void fillRandom( BigDogArrayBase<float[]> _array ) {
		var array = (BigDogArray_F32)_array;
		array.applyIdx(0, array.size, ( idx, v ) -> rand.nextFloat());
	}

	@Override public Object get( BigDogArrayBase<float[]> _array, int index ) {
		var array = (BigDogArray_F32)_array;
		return array.get(index);
	}

	@Override public void copy( BigDogArrayBase<float[]> _src, BigDogArrayBase<float[]> _dst ) {
		var src = (BigDogArray_F32)_src;
		var dst = (BigDogArray_F32)_dst;
