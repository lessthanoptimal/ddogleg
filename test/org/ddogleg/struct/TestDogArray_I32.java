...
public class TestDogArray_I32 extends ChecksDogArrayPrimitive<DogArray_I32> {
	@Test void range() {
		DogArray_I32 alg = DogArray_I32.range(-1, 20);
	...
	@Test void resize_default() {
		var alg = new DogArray_I32(2);
		assertEquals(0, alg.size);
		alg.resize(12, (int)1);
		assertTrue(alg.data.length >= 12);
		assertEquals(12, alg.size);
		for (int i = 0; i < alg.size; i++) {
			assertEquals((int)1, alg.get(i));
		}
		...
		alg.resize(20, (int)3);
		for (int i = 0; i < alg.size; i++) {
			assertEquals((int)(i < 10 ? 1 : 3), alg.get(i));
		}
	}
	...
	@Test void resize_operator() {
		var alg = new DogArray_I32(2);
		assertEquals(0, alg.size);
		alg.resize(12, ( idx ) -> (int)idx);
		assertTrue(alg.data.length >= 12);
		assertEquals(12, alg.size);
		for (int i = 0; i < alg.size; i++) {
			assertEquals((int)i, alg.get(i));
		}

		// If the size has been reduced then there are no new elements to initialize
		alg.resize(10, ( idx ) -> (int)2);
		assertTrue(alg.data.length >= 10);
		assertEquals(10, alg.size);
		for (int i = 0; i < alg.size; i++) {
			assertEquals((int)i, alg.get(i));
		...
		// It's now larger and the new elements should be initialized
		alg.resize(20, ( idx ) -> (int)(100 + idx));
		for (int i = 0; i < alg.size; i++) {
			assertEquals((int)(i < 10 ? i : 100 + i), alg.get(i));
		}
	}
	...
	@Override public DogArray_I32 declare( int maxsize ) {
		return new DogArray_I32(maxsize);
	}

	@Override public void push( DogArray_I32 queue, double value ) {
		queue.push((int)value);
	}

	@Override public void insert( DogArray_I32 queue, int index, double value ) {
		queue.insert(index, (int)value);
	}

	@Override public void check( DogArray_I32 queue, int index, double value ) {
	...
	@Test void forEach() {
		DogArray_I32 alg = DogArray_I32.array(1, 2, 3, 4, 5);
		var cpy = new DogArray_I32(alg.size);
		alg.forEach(cpy::add);
