...
public class TestDogArray_I16 extends ChecksDogArrayPrimitive<DogArray_I16> {
	@Test void range() {
		var alg = DogArray_I16.range(-1, 20);
	...
	@Test void count() {
		var alg = DogArray_I16.array(0, 0, 31000, 31000, 31000);

		assertEquals(2, alg.count(0));
		assertEquals(3, alg.count(31000));
	}

	@Test void isEquals() {
		var alg = DogArray_I16.array(0, 0, 1, 1, 4);
	...
	@Test void addAll_queue() {
		var queue0 = new DogArray_I16(2);
		var queue1 = new DogArray_I16(3);
	...
	@Test void resize_default() {
		...
		assertEquals(10, alg.size);
		for (int i = 0; i < alg.size; i++) {
			assertEquals((short)1, alg.get(i));
		}

		// It's now larger and the new elements should be initialized
		alg.resize(20, (short)3);
		for (int i = 0; i < alg.size; i++) {
			assertEquals((short)(i < 10 ? 1 : 3), alg.get(i));
		}
	}
	...
	@Test void resize_operator() {
		var alg = new DogArray_I16(2);
		assertEquals(0, alg.size);
		alg.resize(12, ( idx ) -> (short)idx);
		assertTrue(alg.data.length >= 12);
		assertEquals(12, alg.size);
		for (int i = 0; i < alg.size; i++) {
			assertEquals((short)i, alg.get(i));
		}

		// If the size has been reduced then there are no new elements to initialize
		alg.resize(10, ( idx ) -> (short)2);
		assertTrue(alg.data.length >= 10);
		assertEquals(10, alg.size);
		for (int i = 0; i < alg.size; i++) {
			assertEquals((short)i, alg.get(i));
		...
		// It's now larger and the new elements should be initialized
		alg.resize(20, ( idx ) -> (short)(100 + idx));
		for (int i = 0; i < alg.size; i++) {
			assertEquals((short)(i < 10 ? i : 100 + i), alg.get(i));
		}
	}
	...
	@Test void setTo_array() {
		...
		short[] array = new short[]{1, 3, 4, 5, 7};
	...
	@Test void remove_swap() {
		var alg = DogArray_I16.array(0, 0, 0, 0, 1);
		alg.removeSwap(1);
	...
	@Test void removeRangeM() {
		...
		alg.applyIdx(( idx, val ) -> (short)idx);
	...
	@Override public DogArray_I16 declare( int maxsize ) {
		return new DogArray_I16(maxsize);
	}

	@Override public void push( DogArray_I16 queue, double value ) {
		queue.push((short)value);
	}

	@Override public void insert( DogArray_I16 queue, int index, double value ) {
		queue.insert(index, (short)value);
	}

	@Override public void check( DogArray_I16 queue, int index, double value ) {
	...
	@Test void forEach() {
		var alg = DogArray_I16.array(1001, 1002, 1003, 1004, 1005);
		var cpy = new DogArray_I16(alg.size);
		alg.forEach(cpy::add);
	...
	@Test void applyIdx() {
		var alg = DogArray_I16.array(1, 2, 3, 4, 5);
