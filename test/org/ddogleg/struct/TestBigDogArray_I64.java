 ...
 * @author Peter Abeles
 */
public class TestBigDogArray_I64 extends ChecksBigDogArray<long[]> {
	...
	private void checkResizeValue( BigDogGrowth growth ) {
		...
		alg.resize(blockSize - 4, (long)4);
		alg.resize(blockSize - 1, (long)3);
		...
		alg.fill(0, alg.size, (long)5);
		// Resize past the second block
		alg.resize(blockSize*2 + 1, (long)6);
	...
	@Test void forEach() {
		...
		DogArray_I64 results = new DogArray_I64();
	...
	@Test void forIdx() {
		...
		DogArray_I64 results = new DogArray_I64(12);
		results.resize(12);
	...
	@Test void applyIdx() {
		...
		alg.applyIdx(1, 12, ( idx, val ) -> (long)(idx*2));
		for (int i = 0; i < alg.size; i++) {
			if (i >= 1 && i < 12) {
	...
	@Test void processByBlock() {
		...
		alg.processByBlock(1, alg.size - 1, ( block, idx0, idx1, offset ) -> {
			for (int i = idx0; i < idx1; i++) {
				block[i] = (long)(i - idx0 + offset);
			}
		});
		for (int i = 0; i < alg.size; i++) {
			if (i >= 1 && i < alg.size - 1) {
				assertEquals((long)(i - 1), alg.get(i));
			} else {
				assertEquals(0, alg.get(i));
			}
		}
	...
	@Test void isEquivalent() {
		...
		var a = new BigDogArray_I64(10, 5, BigDogGrowth.GROW);
		var b = new BigDogArray_I64(10, 4, BigDogGrowth.GROW);
	...
	@Override public void fillRandom( BigDogArrayBase<long[]> _array ) {
		var array = (BigDogArray_I64)_array;
		array.applyIdx(0, array.size, ( idx, v ) -> rand.nextLong());
	}

	@Override public Object get( BigDogArrayBase<long[]> _array, int index ) {
		var array = (BigDogArray_I64)_array;
		return array.get(index);
	}

	@Override public void copy( BigDogArrayBase<long[]> _src, BigDogArrayBase<long[]> _dst ) {
		var src = (BigDogArray_I64)_src;
		var dst = (BigDogArray_I64)_dst;
