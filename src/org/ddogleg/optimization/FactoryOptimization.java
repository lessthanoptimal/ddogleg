...
public class FactoryOptimization {
	 ...
	 */
	public static UnconLeastSqTrustRegionSchur_F64<DMatrixRMaj> doglegSchur( @Nullable ConfigTrustRegion config ) {
		if (config == null)
	...
	public static QuasiNewtonBFGS_to_UnconstrainedMinimization quasiNewtonBfgs( @Nullable ConfigQuasiNewton config ) {
		...
		LineSearch lineSearch = switch (config.lineSearch) {
			case FLETCHER86 -> {
			...
			default -> throw new RuntimeException("Unknown line search. " + config.lineSearch);
		};
	 ...
	 */
	public static UnconLeastSqTrustRegion_F64<DMatrixRMaj> dogleg( @Nullable ConfigTrustRegion config ) {
		if (config == null)
			config = new ConfigTrustRegion();
	...
	public static UnconLeastSqTrustRegion_F64<DMatrixRMaj> cauchy( @Nullable ConfigTrustRegion config ) {
		...
		if (config.solverType != LinearSolverType.DEFAULT) {
			throw new IllegalArgumentException("Linear solvers are not used by cauchy");
		}
	...
	public static UnconMinTrustRegionBFGS_F64 doglegBFGS( @Nullable ConfigTrustRegion config ) {
		...
		if (config.solverType != LinearSolverType.DEFAULT) {
			throw new IllegalArgumentException("Linear solvers are not used by BFGS");
		}
