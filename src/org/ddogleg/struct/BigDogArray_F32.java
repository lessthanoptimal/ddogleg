...
/**
 * Implementation of {@link BigDogArray} for float[].
 *
 * @author Peter Abeles
 */
public class BigDogArray_F32 extends BigDogArray<float[]> {

	// WARNING: BigDogArray_F32 is a template for all the other BigDogArray classes. Only modify BigDogArray_F32.
	...
	public BigDogArray_F32( int initialAllocation, int blockSize, Growth growth ) {
		super(initialAllocation, blockSize, growth);
	}

	@Override protected float[] newArrayInstance( int size ) {
		return new float[size];
	}

	@Override protected int arrayLength( float[] array ) {
		return array.length;
	}
