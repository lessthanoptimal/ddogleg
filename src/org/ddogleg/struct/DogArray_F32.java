...
public class DogArray_F32 implements DogArrayPrimitive<DogArray_F32> {
	...
	public boolean isEquals( DogArray_F32 values, float tol ) {
		if (size != values.size)
			return false;
		for (int i = 0; i < size; i++) {
	 ...
	 */
	public void get( int offset, float[] dst, int dstOffset, int length ) {
	...
	public void unsafe_set( int index, float value ) {
		data[index] = value;
	}
	 ...
	 */
	public int indexOf( float value, int startIndex ) {
