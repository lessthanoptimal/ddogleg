...
public class DogArray_I16 implements DogArrayPrimitive<DogArray_I16> {
	 ...
	 */
	public void get( int offset, short[] dst, int dstOffset, int length ) {
	...
	public void setTail( int index, int value ) {
		checkBounds(index);		data[size - index - 1] = (short)value;
	}
	...
	@Override public DogArray_I16 setTo( DogArray_I16 original ) {
		resize(original.size);
		System.arraycopy(original.data, 0, data, 0, size());
		return this;
	}

	@Override public DogArray_I16 resize( int size ) {
		reserve(size);
		this.size = size;
		return this;
	}
	...
	@Override public DogArray_I16 reserve( int amount ) {
		if (amount < 0)
	...
	@Override public void removeRangeM( int idx0, int idx1 ) {
		if (idx1 < idx0)
