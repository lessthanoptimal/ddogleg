...
/**
 * Implementation of {@link BigDogArrayBase} for byte[].
 *
 * @author Peter Abeles
 */
public class BigDogArray_I8 extends BigDogArrayBase<byte[]> {
	...
	@Override public void removeSwap( int index ) {
		...
		byte target = get(index);
		byte tail = get(indexTail);
	...
	/**
	 * Appends a single value to the end of the array. identical to {@link #append(byte)}.
	 *
	 * @param value (Input) The new value which is to be added
	 */
	 ...
	 * of each other.
	 */
	public boolean isEquivalent( BigDogArray_I8 o ) {
		if (size != o.size)
			return false;
