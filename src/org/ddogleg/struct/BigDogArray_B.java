...
/**
 * Implementation of {@link BigDogArray} for boolean[].
 *
 * @author Peter Abeles
 */
public class BigDogArray_B extends BigDogArray<boolean[]> {

	// WARNING: BigDogArray_B is a template for all the other BigDogArray classes. Only modify BigDogArray_B.
	...
	public BigDogArray_B( int initialAllocation, int blockSize, Growth growth ) {
		super(initialAllocation, blockSize, growth);
	}

	@Override protected boolean[] newArrayInstance( int size ) {
		return new boolean[size];
	}

	@Override protected int arrayLength( boolean[] array ) {
		return array.length;
	}
