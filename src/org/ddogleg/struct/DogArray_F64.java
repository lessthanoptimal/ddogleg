...
public class DogArray_F64 implements DogArrayPrimitive<DogArray_F64> {
	 ...
	 */
	public void get( int offset, double[] dst, int dstOffset, int length ) {
	...
	@Override public DogArray_F64 setTo( DogArray_F64 original ) {
		resize(original.size);
		System.arraycopy(original.data, 0, data, 0, size());
		return this;
	}

	@Override public DogArray_F64 resize( int size ) {
		reserve(size);
		this.size = size;
		return this;
	}
	...
	@Override public DogArray_F64 reserve( int amount ) {
		if (amount < 0)
