 ...
 * @author Peter Abeles
 */
public class EvalFunctionBundle2D_DSCC implements
		EvalFuncLeastSquaresSchur<DMatrixSparseCSC> ,
		EvalFuncLeastSquares<DMatrixSparseCSC>
{
	...
	public EvalFunctionBundle2D_DSCC(long seed , double length, double depth,
									 int numCamera, int numLandmarks)
	{
		...
		for (int i = 0; i < optimal.length; i++) {
			initial[i] = optimal[i] + rand.nextGaussian()*0.15;
		}
	...
	public FunctionNtoMxN<DMatrixSparseCSC> getJacobian() {
		return new SchurJacobian_to_NtoMxN(getJacobianSchur());
	}
	...
	private void decode(double[] input, List<Point2D> cameras , List<Point2D> landmarks ) {
		cameras.clear();
		landmarks.clear();
		int index = 0;
		for (int i = 0; i < numCamera; i++) {
