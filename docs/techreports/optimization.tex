...
Variable scaling can refer to several different parts of the non-linear optimization problem. Here we will discuss scaling of the input variables $\bm{x}$ and scalling of the Hessian $B_k$ internally. Thoughout the literature, correct scaling, in all of its forms, is emphansized as an essential task and that you are a bad person doomed to failure if you skip it. 
...
of all the variables $\bm{x}$ being optimzied are the same and around one. Input scaling must be done by the user because DDogleg has no knowledge of the units. When solving linear problems, as is often done to initialize non-linear methods, input scaling is absolutely neccisary and quick experiments can show that. It's also the step most often skipped by people because of the extra work involved.
...
For Hessian Scaling the Hessian matrix is rescalled so that the diagonal elements are approximately one. We will discuss Hessian scaling in regards to Trust Region (and Levenberg-Mardquardt) methods since those are the only methods in DDogleg where it is applied.
...
\begin{algorithmic}[1]
	...
	\While{$k < k_{\mbox{max}}$ and not $done$}
		...
		\Else
			\State $\lambda \gets \nu \lambda$ \Comment{Emphansize the gradient more}
			\State $\nu = 2\nu$
		\EndIf
		\State $k \gets k + 1$
	\EndWhile
