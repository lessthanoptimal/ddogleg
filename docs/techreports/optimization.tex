...
\begin{algorithmic}[1]
	...
	\State $\nu \gets \delta_f / \delta_f$ \Comment{Score Ratio} 
	\If{ $\delta_f \le 0$ or $\nu <\frac{1}{4}$} \Comment{Fit score got worse or the model poor?}
		\State $\Delta_{k+1} \gets \frac{1}{2}\Delta_k$
	\Else
		\If{$\nu>\frac{3}{4}$}
			\Comment{See if it should increase the region size}
...
Correctly scaled variables is critical to high performance, especially for large scale problems. When variable are correctly scaled their values will on average have the same order of magnitude. Mixing very large (e.g. 1e8) with very small (e.g. 1e-12) numbers causes significant round off errors.

Scale adjustment is typically implemented using a diagonal matrix $\bm{D}$ with positive elements. This matrix is then applied to $p$ to created its scaled version $\tilde{p} = \bm{D}p$. The trust region is no longer a circle but an ellipse \cite{numopt2006}, resulting in this alternative trust region subproblem:
\begin{equation}
\begin{array}{lr}
\min\limits_{p\in \R^n} m_k(\bm{p}) = f_k + g^T_k \bm{p} + \frac{1}{2}\bm{p}^T \bm{B}_k \bm{p} & s.t. \norm{\bm{D}p} \le \Delta_k
\end{array}
\end{equation}
As suggested in \cite{numopt2006} this is implemented internally by substituting $\tilde{p}$ for $p$, $D^{-1}g_k$ for $g_k$, and $D^{-1}B_k D^{-1}$ for $B_k$. 

When considering scaling you should also be aware that scaling will change the steepest descent direction but not the solution to the Newton problem \cite{dennis1996}. In effect it changes the emphansis that it places on variables when trying to minimize them.

DDogleg offers the following options:
\begin{enumerate}
\item Apply no scaling
\item Automatically select a scaling from the Hessian at every iteration
\item User specifies a fixed scaling
\end{enumerate}

Automatic scaling is typically found using second derivatives $\frac{\partial^2 f}{\partial x^2_i}$ found in the Hessian's diagonal elements. The specific formula used in DDogleg is as follows
\begin{equation}
D_k^{ii} = \max( d_{\bigtriangledown},\min( \sqrt{|B_k^{ii}|} , d_{\bigtriangleup} ))
\end{equation}
where $d_{\bigtriangledown}$ is the minimum allowed scaling value and $d_{\bigtriangleup}$ is the maximum. This approach can handle negative definite $B_k$ and has the desirable property \cite{dennis1996} that the diagonal elements in $\tilde{B}_k = D^{-1}B_k D^{-1}$ will typically be $\tilde{B}_k^{ii} \approx 1$, unless clamped or $B_k^{ii}$ is zero. Matrices with this property are properly scaled can be decomposed with minimal numerical issues. When configured to automatically scale DDogleg will update its scaling at every iteration. This is mostly unnecissary but it's also not very expensive.

There are two primary ways for the user to specify their own scaling. 1) They can pre-scale the data and adjust their forumlas. 2) They can provide a scaling vector which contains the diagonal elements in $D$. The former is arguably the best way while the later is easier. If the former is done then automatic scaling can still be applied, ensuring that the Hessian matrix is well suited for decomposition. A third option, for advanced users, is to override a class in DDogleg and change the scaling to any arbitrary function you wish.
...
\item Accept your fate, read the literature, checkout DDogleg's source code, and start trying to understand why it's failing by stepping through the code line by line when your program runs.
\end{enumerate}

\subsection{Trust Region: Initial Region Size} 
\label{section:init_region_size}

Selection of the initial trust region size is important but typically not discussed in reference material \cite{fletcher1987,numopt2006,IMM2004}. In those references it's simply a tuning parameter that the user is supposed to select. In practice people arbitrarily select values such as one.

Since the trust region size is dynamically adjusted, Trust Region algorithms are somewhat invariant to poor selection of the initial size. However, there are stituations where a poorly chosen initial value can cause poor performance. One observed failure mode is with the Dogleg method. If the initial value is too small it will default to using the Cauchy Point. The Cauchy Point can lead it down a poor path and cause it to get stuck in a local minimum. Setting to a large of an initial value is also problematic. First it's not clear what would a large value is for all problems and setting it to a value which is too large can take numerious iterations to fix.

To overcome this difficulty, by default, DDogleg will use an automatic algorithm for selecting the initial region size, summarized below:
\begin{algorithm}{}
...
\end{algorithmic}
\end{algorithm}
Based on test problems included with DDogleg this simple automatic approach results in fewer convergence issues than selecting the initial region value to fixed sizes such as 1 or 100. In DDogleg initial region sizes can also be manually specified.
