...
\hline
Quasi-Newton BFGS & Quadratic & ? & ? & ? & ? \rule{0pt}{2.6ex} \\
Trust Region Cauchy & Linear & Yes & Yes & Yes & Yes \\
Trust Region Dogleg & Quadratic & [1] & [1] & Yes & Yes\\[1ex]
\hline
\multicolumn{6}{l}{
...
%\cite{numopt2016}
The solution to this problem (see for details and diagrams) is as follows
\begin{equation}
...
\end{cases}
\end{eqnarray}
where $B$ is positive definite, and $p^{dog}$ is the point selected by the Dogleg method. The solution to $\tau$ can be easily found by solving along each line segment. If $B$ is not positive definite then the Cauchy Point algorithm is used instead. Alternative approaches for handling this situation are discussed in \cite{numopt2016} but have not yet been added to DDogleg.
